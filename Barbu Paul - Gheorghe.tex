...
%grid.arrange(autoplot(usgdp) + scale_x_continuous(breaks=seq(1950, 2020, 20)) + xlab("Year") + ylab("US Dollars") + ggtitle("Under-differenced data"), ggAcf(usgdp) + ggtitle(""), ggPacf(usgdp) + ggtitle(""), 
%autoplot(diff(usgdp)) + scale_x_continuous(breaks=seq(1950, 2020, 20)) + ylab("") + xlab("Year") + ggtitle("Autoregressive data"), ggAcf(diff(usgdp)) + ggtitle(""), ggPacf(diff(usgdp)) + ggtitle(""), 
%autoplot(diff(usgdp, differences=2)) + scale_x_continuous(breaks=seq(1950, 2020, 20)) + ylab("") + xlab("Year") + ggtitle("White Noise"), ggAcf(diff(usgdp, differences=2)) + ggtitle(""), ggPacf(diff(usgdp, differences=2)) + ggtitle(""),
%autoplot(diff(usgdp, differences=3)) + scale_x_continuous(breaks=seq(1950, 2020, 20)) + ylab("") + xlab("Year") + ggtitle("Moving average data"), ggAcf(diff(usgdp, differences=3)) + ggtitle(""), ggPacf(diff(usgdp, differences=3)) + ggtitle(""),
%autoplot(diff(usgdp, differences=4)) + scale_x_continuous(breaks=seq(1950, 2020, 20)) + ylab("") + xlab("Year") + ggtitle("Over-differenced data"), ggAcf(diff(usgdp, differences=4)) + ggtitle(""), ggPacf(diff(usgdp, differences=4)) + ggtitle(""),
%layout_matrix = cbind(c(1,2,3), c(4, 5,6), c(7, 8, 9), c(10, 11, 12), c(13, 14, 15)))
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{darmadynamics3}
...
\begin{figure}[htbp]
    ...
    \caption{Prediction by the best model on the \texttt{Ph1} data (sampled every 5 minutes)}
    \label{dbestph1}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Prediction by the best model on the \texttt{Ph2} data (sampled every 5 minutes)}
    \label{dbestph2}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Prediction by the best model on the \texttt{Ph3} data (sampled every 5 minutes)}
    \label{dbestph3}
\end{figure}
...
What is fundamentally different though is the time covered by the dummies. Here the interval is directly proportional with the interval the sun shines most, which, as shown here by the best model is between 11:00 in the morning and 17:00 in the afternoon (start dummy is 11, and length is 6).
...
\begin{figure}[htbp]
    ...
    \caption{Prediction by the best model on the \texttt{PV1} data (sampled every 5 minutes)}
    \label{dbestpv1}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Prediction by the best model on the \texttt{PV2} data (sampled every 5 minutes)}
    \label{dbestpv2}
\end{figure}
...
%        \textbf{ARIMA}             & 225.91       & 220.00       & 218.04       & 161.40       & 183.96       & 201.86        \\ \hline
        \textbf{ARIMA}             & 225.91       & 219.01       & 215.51       & 157.91       & 172.99       & 198.27        \\ \hline
    \end{tabular}
...
%df <- data.frame(Model=rep(c('Markov', 'Neural', 'ARIMA'), each=6), Data=rep(c("PV1", "PV2", "Ph1", "Ph2", "Ph3", "Mean"), 3),
%MAE=c(27.29,	26.16,	42.39,	25.18,	51.11, 34.426, 111.71,	107.16,	279.99,	199.35,	357.14, 211.07, 225.91,	220.00,	218.04,	161.40,	183.96, 201.86))
%head(df)
%
%p<-ggplot(data=df, aes(x=Data, y=MAE, fill=Model)) +
